...
export const columnsOld: ColumnDef<ColumnsDataTableOld>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="capitalize w-28">{row.original.EquipmentType.name}</div>
      );
    },
  },
  {
            ...
            className={cn(
              'capitalize hover:cursor-pointer text-foreground opacity-90 w-24 flex justify-center',
              {
                ...
                'bg-green-500 hover:opacity-100 hover:bg-green-500':
                  row.original.flow === 'checkIn',
              },
            )}
          >
            {row.original.flow === 'checkIn' && 'Check In'}
            {row.original.flow === 'checkOut' && 'Check Out'}
          </Badge>
        </div>
      );
    },
