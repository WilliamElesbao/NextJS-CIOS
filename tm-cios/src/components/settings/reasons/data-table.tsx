...
import { deleteReason, editReason } from '@/lib/actions';
...
export const columns: ColumnDef<ReasonsType>[] = [
  {
    accessorKey: 'name',
    header: 'Nome',
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const [equipmentToEdit, SetEquipmentToEdit] =
        useState<ReasonsType | null>(null);
      ...
      const onSave = async () => {
        const result = await editReason(equipmentToEdit!);
        if (!result.status) {
          return toast({
      ...
      const onDelete = async () => {
        const result = await deleteReason(equipmentToDelete!.id!);
        if (!result.status) {
          return toast({
...
export function DataTable({ data }: { data: ReasonsType[] }) {
