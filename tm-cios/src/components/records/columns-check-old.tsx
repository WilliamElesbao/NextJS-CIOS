...
import { ColumnsDataTableOld } from '@/lib/definitions';
import {
  capitalizeName,
  formatLongDate,
  getFirstAndLastName,
...
export const columnsOld: ColumnDef<ColumnsDataTableOld>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="w-28 capitalize">{row.original.EquipmentType.name}</div>
      );
    },
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="w-28 capitalize">{row.original.patrimonyNumber}</div>
      );
    },
  },
  {
    ...
    cell: ({ row }) => {
      return <div className="w-28 uppercase">{row.original.description}</div>;
    },
  },
  {
